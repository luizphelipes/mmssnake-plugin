<?php
/**
 * Teste Robusto de Criação de Pedido
 */

// Carregar WordPress
require_once('../../../wp-load.php');

echo "<h2>🧪 Teste Robusto de Criação de Pedido</h2>";
...
echo "<hr><h3>🧪 Testando Criação de Pedido com Hook</h3>";
...
try {
    ...
    foreach ($order->get_items() as $item) {
        wc_add_order_item_meta($item->get_id(), 'Instagram', 'teste_robusto', true);
    ...
    // Definir status (importante para disparar hooks)
    ...
    echo "<p>✅ Status definido como: <strong>{$order->get_status()}</strong></p>";
    
    // Aguardar um pouco para o processamento
    echo "<p>⏳ Aguardando processamento automático...</p>";
    sleep(2);
    ...
    if ($pedido_inserido) {
        ...
        if (empty($pedido_inserido->service_id_pedido)) {
            ...
            if (file_exists($log_file)) {
                ...
                $log_lines = explode("\n", $log_content);
                $recent_logs = array_slice($log_lines, -30); // Últimas 30 linhas
                
                echo "<div style='background: #f5f5f5; padding: 10px; border: 1px solid #ddd; max-height: 400px; overflow-y: auto; font-family: monospace; font-size: 12px;'>";
        ...
        // Verificar se o hook foi disparado
        echo "<hr><h3>🔍 Verificando Hooks</h3>";
        echo "<p>📝 Verificando se o hook foi disparado...</p>";
        
        // Verificar logs para ver se o hook foi executado
        $log_file = plugin_dir_path(__FILE__) . 'debug-pedidos-plugin.log';
        if (file_exists($log_file)) {
            echo "<p>📋 Últimas linhas do log:</p>";
            $log_content = file_get_contents($log_file);
            $log_lines = explode("\n", $log_content);
...
echo "<li>🧪 Pedido de teste criado com status</li>";
